...
export default function Inventory() {
  ...
  const { inventoryWatchData, userInfoData, cartInfoData, loading } =
    useSelector((state) => ({
      ...state.products,
    }));
  useEffect(() => {
    dispatch(inventoryWatch());
    dispatch(userInfo());
  }, [dispatch]);

  const inventoryWatchListData = inventoryWatchData[0];
  const cartData = cartInfoData[0];
  const orderItems = cartData?.orderItems;
  const data = inventoryWatchData[0]?.products;
  ...
  return (
    <SafeAreaView style={styles.safeAreaContainer}>
      ...
      <View style={styles.heading}>
        <Text style={styles.headingText}>Your Top Purchases</Text>
      </View>
